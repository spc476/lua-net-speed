...
static int	socklua_recvb		(lua_State *const) __attribute__((nonnull));
...
/***********************************************************************
*
*	remaddr,err = sock:recvb(buffer[,timeout = inf])
*
*	sock    = net.socket(...)
...
static int socklua_recvb(lua_State *const L)
{
  sockaddr_all__t *remaddr;
  socklen_t        remsize;
  sock__t         *sock;
  buffer__s       *buffer = luaL_checkudata(L,2,TYPE_BUFFER);
  ssize_t          bytes;
  ...
  if (lua_isnumber(L,3))
  {
    int           timeout = (int)(lua_tonumber(L,3) * 1000.0);
    struct pollfd fdlist;
    int           rc;
  ...
  bytes = recvfrom(sock->fh,buffer->buf,buffer->len,0,&remaddr->sa,&remsize);
  if (bytes < 0)
  {
    lua_pushnil(L);
    lua_pushinteger(L,errno);
...
int luaopen_net(lua_State *const L)
{
